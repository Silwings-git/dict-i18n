package cn.silwings.dicti18n.starter.advice;
 ...
 * <ul>
 *     <li>{@link DictI18nResponseEnhancerProperties} global configuration</li>
...
public class DictI18nResponseEnhancer implements ResponseBodyAdvice<Object> {
    ...
    private final DictI18nResponseEnhancerProperties properties;
    ...
    public DictI18nResponseEnhancer(final DictI18nProcessor processor,
                                    ...
                                    final DictI18nResponseEnhancerProperties properties) {
    ...
    public boolean supports(final MethodParameter returnType, final Class<? extends HttpMessageConverter<?>> converterType) {
        ...
        if (!this.properties.getIncludePackages().isEmpty()) {
            final String className = returnType.getParameterType().getName();
            final boolean matched = this.properties.getIncludePackages().stream().anyMatch(className::startsWith);
            if (!matched) {
                return false;
            }
        }
        ...
        for (String annotationClassName : this.properties.getExcludeAnnotations()) {
            try {
